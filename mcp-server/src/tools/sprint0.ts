...
export function registerSprint0Tools(server: McpServer) {

  server.tool(
    "audit_site_content",
    "Site-wide audit — tìm entity thiếu content, SEO, translations. Gọi đầu tiên để nắm tình trạng.",
    {
  ...
  server.tool(
    "list_entities",
    "List entities theo loại với filter. Dùng để xem danh sách cần xử lý.",
    {
  ...
  server.tool(
    "search_entities",
    "Tìm entity theo keyword. Dùng khi biết tên sản phẩm/danh mục nhưng không biết slug.",
    {
  ...
  server.tool(
    "get_review_queue",
    "Lấy danh sách draft MCP chưa được review. Dùng để biết việc gì đang chờ xử lý.",
    {
