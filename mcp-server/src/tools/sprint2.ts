...
export function registerSprint2Tools(server: McpServer) {

  server.tool(
    "get_category_context",
    "Load full category context trước khi viết content. Gọi đầu tiên.",
    {
  ...
  server.tool(
    "check_category_readiness",
    "Kiểm tra category đủ điều kiện activate chưa. Gọi trước activate_category.",
    {
      slug: z.string().describe("Category slug"),
    },
  ...
  server.tool(
    "save_category",
    "Upsert category — tạo mới hoặc update content, translations, SEO, GEO/FAQ.",
    {
  ...
  server.tool(
    "activate_category",
    "Activate category sau khi readiness pass. Observer tự sync JSON-LD + Sitemap.",
    {
      slug: z.string().describe("Category slug"),
    },
